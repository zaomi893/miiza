...
- `fastbuild_6.12.38_oneplus_15t_clean.yml`：一加 15T 干净可启动基线，只加入固定 ReSukiSU root，不加入风驰 BTF、序列号锁或其他功能；通过 uname、ARM64 Image、`CONFIG_LTO_NONE`、`CONFIG_KSU` 和 BTF 配置检查后再上传刷机包。
