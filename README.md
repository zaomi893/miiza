...
每次构建生成独立的 32 字节随机盐和一次性启动标记令牌，将 `SHA-256(随机盐 || 序列号)` 写入 Image，不保存明文序列号。刷入时不校验设备序列号：AK3 会先清除当前槽 `init_boot` 中的旧标记并回读确认，然后安装仅供本构建使用的首启助手。第一次在不匹配设备上启动时仍允许进入系统；助手只有在成功写入并回读验证本构建标记后才通知内核，内核从开机起满 180 秒时重启。下一次仍不匹配且标记令牌一致时，内核在执行 Android `/init` 前停止启动。序列号匹配时始终正常启动并忽略任何标记；风驰 BTF 兼容入口仍要求序列号已经验证。
