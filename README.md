...
一加 15（SM8850/canoe）Android 16 **只编内核**构建项目：从官方源码构建自定义内核，
**保留官方 vendor_dlkm 不动**——风驰内核调速器（HMBIRD II）的完整依赖链
...
Kleaf 构建 `//msm-kernel:canoe_perf_images`（内核 + vendor_boot.img，绕开 oplus 模块图）→ 打包为可刷 zip。
...
而官方 vendor_dlkm 本来就在手机上、且已含完整风驰链路。只要：
1. 用 `canoe_perf` 配置（与官方一致）构建内核 → KMI / 内核版本与官方模块匹配；
2. 刷入含自编内核的 vendor_boot.img，保留官方 vendor_dlkm。
风驰内核调速器就能继续工作，且完全绕开"108 个 oplus 模块未开源"的构建阻塞（那是重编 vendor_dlkm 才需要面对的问题）。
## 内核在哪个分区？——vendor_boot，不是 boot
`canoe_perf` 的 `kernel_images` 配置为 `build_vendor_boot=True`、`build_vendor_kernel_boot=False`、**无 `boot_img`/`init_boot_img`**，
因此该机型**内核 Image 在 vendor_boot 分区**，构建产物是 `vendor_boot.img`（内含自编内核 + 同源 vendor ramdisk + DTB），
**没有独立的 boot.img**。官方 vendor_dlkm（风驰模块）是独立分区，保持官方原样即可。
...
**这些只影响全量 vendor_dlkm 构建；内核-only 路线（`canoe_perf_images`）不受影响**——该宏的规则在加载期被创建
但从未被分析，仅要求 `oplus_modules.bzl` 保持官方原样可解析，因此 assemble.sh **不再运行** trim_public_oplus.py。
## 构建流程
```
...
build.sh               Kleaf 构建 //msm-kernel:canoe_perf_images -> artifacts/vendor_boot.img
package.sh             打包 vendor_boot.img -> oneplus15-hmbird2-*.zip（AnyKernel3，只刷 vendor_boot）
```
## 刷机（scripts/package.sh）
构建成功后运行 `bash scripts/package.sh artifacts <name>.zip`，产出只刷 **vendor_boot.img** 的 AnyKernel3 zip。
**不刷 vendor_dlkm / dtbo**，vendor_dlkm 保留官方原厂（风驰内核调速器在官方 vendor_dlkm 中）。
刷入前提：**Bootloader 已解锁**（一加 15 解锁后为 Orange State）。两种方式任选：
- **自定义 Recovery / KernelSU Flasher**：直接刷入生成的 `oneplus15-hmbird2-*.zip`。
- **fastboot 直刷**（等价）：`fastboot flash vendor_boot vendor_boot.img`。
## 备用：完整 vendor_dlkm 构建（scripts/trim_public_oplus.py）
如果确需从公开源码重建完整 vendor_dlkm（不依赖官方分区），可参考 `scripts/trim_public_oplus.py`
（当前 assemble.sh **不执行**它，因为内核-only 路线不需要裁剪且旧版重写有 Starlark 语法 bug）：
...
若启用此路线需先修复该脚本的引号重包裹 bug，并把 build.sh 目标改回 `//msm-kernel:canoe_perf_dist`。
## 仍未覆盖的缺口
- 内核与官方模块的 KMI 匹配依赖同一分支同一配置；若官方 vendor_dlkm 与所选分支版本不一致，模块可能无法加载。
- vendor_boot.img 用 AOSP 测试密钥签名（AVB testkey），仅限已解锁 Bootloader 使用。
- 其余 Kleaf 构建期问题（如 Kconfig 去重、DTB 头文件、NDK zip 获取等）沿用仓库既有补丁。
